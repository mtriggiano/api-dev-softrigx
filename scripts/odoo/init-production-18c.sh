...
# 🚀 Script de creación de instancia Odoo 18 Community - Versión refactorizada
...
REPO=$(get_repo_path "18" "community")
PYTHON=$(get_python_bin "18" "community")

if [[ -z "$REPO" ]] || [[ ! -f "$REPO" ]]; then
    echo "❌ ERROR: No se pudo obtener la ruta del repositorio Odoo 18 Community"
...
# Detectar si hay una subcarpeta (como odoo18c-temp/) o si los archivos están en la raíz
...
echo "[Unit]
Description=Odoo 18c Instance - $INSTANCE_NAME
